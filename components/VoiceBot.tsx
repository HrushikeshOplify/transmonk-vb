...
export default function VoiceBot() {
  ...
  //time limit
  const sessionLimitSentRef = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    const handleTranscripts = () => {
      ...
      setStats((prev) => ({
        ...prev,
        exchanges: current.filter((t) => t.speaker === "user").length,
      }));
  ...
  const startDurationTimer = () => {
    startTimeRef.current = Date.now();
    sessionLimitSentRef.current = false;
    durationIntervalRef.current = setInterval(() => {
  ...
  const cleanup = () => {
    ...
    setTranscripts([]);
    setStats({ exchanges: 0, ragQueries: 0 });
    sessionLimitSentRef.current = false; // reset for next call
  };
  ...
  const latestAgentMessage =
    [...transcripts].reverse().find((t) => t.speaker === "agent")?.text || "";
